...
/**
 * Quiz Builder — ویرایش سوالات + تنظیمات یک آزمون خاص.
 *
 * Props تغییر کرد (چندآزمونی):
 *   قبلاً: courseId  → هر بار کل آزمون دوره جایگزین می‌شد
 *   حالا:  quizId   → هر آزمون مستقل است؛ فقط همین آزمون تحت تأثیر قرار می‌گیرد
 *
 * جریان کار:
 *   - از QuizManager با «ویرایش» (✏) باز شود → quizId موجود، GET /quizzes/:quizId لود می‌شود
 *   - از QuizManager پس از «ایجاد آزمون» باز شود → quizId تازه‌ساز، بانک خالی
 *   - ذخیره: PUT /quizzes/:quizId (نه POST، نه جایگزینی کل دوره)
 */
interface Props {
  quizId: number;
  onPrev?: () => void;
...
const InstructorQuizQuestions: React.FC<Props> = ({
  ...
  useEffect(() => {
    const load = async () => {
      try {
        // GET /quizzes/:quizId — جزئیات کامل این آزمون خاص
        ...
        if (quiz) {
          ...
          setSettings((s) => ({
            ...
            startDateObj: quiz.StartAt
              ...
              : null,
            startTime: quiz.StartAt
              ? new Date(quiz.StartAt).toTimeString().slice(0, 5)
              : "09:00",
            endDateObj: quiz.EndAt
              ...
              : null,
            endTime: quiz.EndAt
              ? new Date(quiz.EndAt).toTimeString().slice(0, 5)
              : "23:59",
        ...
        // آزمون تازه‌ساز است — بانک خالی، تنظیمات پیش‌فرض
      } finally {
  ...
  const handleCorrectChange = (index: number) => {
    setFormChoices((prev) => prev.map((c, i) => ({ ...c, isCorrect: i === index })));
  };
  ...
  const saveQuestionFromModal = () => {
    ...
    if (!text) { toast.error("متن سوال را وارد کنید."); return; }
    if (choices.some((c) => !c.text)) { toast.error("متن همه گزینه‌ها را وارد کنید."); return; }
    if (choices.filter((c) => c.isCorrect).length !== 1) { toast.error("دقیقاً یک گزینه صحیح انتخاب کنید."); return; }
    if (!formScore || formScore <= 0) { toast.error("نمره سوال باید بزرگتر از صفر باشد."); return; }

    if (editingClientId) {
      setQuestions((prev) =>
        prev.map((q) =>
          q.clientId === editingClientId
            ? { ...q, questionText: text, skillTag: formSkillTag.trim(), choices, score: formScore }
            : q,
        ),
      );
    } else {
      setQuestions((prev) => [
        ...prev,
        { clientId: makeClientId(), questionText: text, skillTag: formSkillTag.trim(), choices, score: formScore, isAiGenerated: false },
      ]);
    }
    setModalOpen(false);
  };
  ...
  // ── AI generation — حالا از quizId (نه courseId) ────────────────────────────
  const handleGenerateAi = async () => {
    ...
    setGenerating(true);
    try {
      // POST /quizzes/:quizId/generate — context پرامپت از Quiz→Course در بک‌اند گرفته می‌شود
      const generated = await quizService.generateQuestions(quizId, aiCount);
      ...
      toast.error(err?.response?.data?.message || "تولید سوالات با خطا مواجه شد.");
    } finally {
      setGenerating(false);
    }
  };

  // ── Save — PUT /quizzes/:quizId (فقط همین آزمون) ────────────────────────────
  const handleSaveQuiz = async () => {
    if (questions.length === 0) { toast.error("حداقل یک سوال اضافه کنید."); return; }
    if (!settings.startDateObj || !settings.endDateObj) { toast.error("تاریخ شروع و پایان آزمون را مشخص کنید."); return; }
    if (settings.questionsToShow > questions.length) {
      toast.error("تعداد سوالات نمایشی نمی‌تواند از تعداد کل سوالات بیشتر باشد.");
      return;
    }

    const totalMaxScore = questions.reduce((sum, q) => sum + (q.score ?? 1), 0);
    if (!settings.passScore || settings.passScore <= 0) { toast.error("نمره قبولی را وارد کنید."); return; }
    if (settings.passScore > totalMaxScore) { toast.error("نمره قبولی نمی‌تواند از مجموع نمرات سوالات بیشتر باشد."); return; }
    ...
    if (new Date(endAt) <= new Date(startAt)) { toast.error("زمان پایان باید بعد از زمان شروع باشد."); return; }

    setSaving(true);
    try {
      // PUT /quizzes/:quizId — فقط همین آزمون ویرایش می‌شود، سایر آزمون‌های دوره دست‌نخورده‌اند
      ...
      toast.error(err?.response?.data?.message || "ذخیره آزمون با خطا مواجه شد.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="form-inner wizard-form-card instructor-quiz-page">
      ...
      <div className="quiz-page-header">
        <div>
          ...
          <div className="quiz-question-count">
            ...
            بانک سوالات: <strong>{questions.length}</strong>
          </div>
        </div>
        ...
        <div className="quiz-header-actions">
          <div className="ai-generator">
            ...
            />
            <button type="button" className="btn quiz-ai-btn" onClick={handleGenerateAi} disabled={generating}>
              <i className="fas fa-magic" />
              {generating ? "در حال تولید..." : "تولید با هوش مصنوعی"}
            </button>
          </div>
          <button type="button" className="btn quiz-add-btn" onClick={openAddModal}>
      ...
      {questions.length === 0 ? (
        <div className="quiz-empty-state">
          ...
          <p>می‌توانید سوالات را به‌صورت دستی اضافه کنید یا از هوش مصنوعی برای تولید سوال استفاده کنید.</p>
          <button type="button" className="btn quiz-modal-save" onClick={openAddModal}>
              ...
              <div className="quiz-question-top">
                ...
                <div className="quiz-question-content">
                  <div className="quiz-question-heading">
                    ...
                    <div className="quiz-question-badges">
                      <span className="quiz-score-badge">
                        <i className="fas fa-star" />{q.score ?? 1} نمره
                      </span>
                      {q.skillTag && (
                        <span className="quiz-skill-badge">
                          <i className="fas fa-tag" />{q.skillTag}
                        </span>
                      )}
                      {q.isAiGenerated && (
                  ...
                  <div className="quiz-choices">
                    {q.choices.map((c, ci) => (
                      <div key={ci} className={`quiz-choice ${c.isCorrect ? "correct" : ""}`}>
                        <span className="quiz-choice-icon">
                          {c.isCorrect ? <i className="fas fa-check" /> : <span>{ci + 1}</span>}
                        </span>
                        <span>{c.text}</span>
                ...
                <div className="quiz-question-actions">
                  <button type="button" className="quiz-icon-btn edit" title="ویرایش سوال" onClick={() => openEditModal(q)}>
                    <i className="isax isax-edit-2" />
                  </button>
                  <button type="button" className="quiz-icon-btn delete" title="حذف سوال" onClick={() => deleteQuestion(q.clientId)}>
          ...
          <div className="row g-3">
            ...
            <div className="col-md-6 col-lg-3">
              ...
              <DatePicker
                ...
                onChange={(val) => setSettings({ ...settings, startDateObj: val as DateObject })}
            ...
            <div className="col-md-6 col-lg-3">
              ...
              <DatePicker
                ...
                onChange={(val) => setSettings({ ...settings, endDateObj: val as DateObject })}
                inputClass="form-control"
                calendarPosition="bottom-right"
                containerStyle={{ width: "100%" }}
              />
            ...
            <div className="col-md-6 col-lg-3">
              ...
              <input
                ...
                onChange={(e) => setSettings({ ...settings, passScore: Number(e.target.value) })}
              />
              <small className="quiz-field-hint">
                از مجموع {questions.reduce((s, q) => s + (q.score ?? 1), 0)} نمره
              </small>
            </div>
            ...
            <div className="col-md-6 col-lg-3">
              ...
              <input
                ...
                onChange={(e) => setSettings({ ...settings, questionsToShow: Number(e.target.value) })}
              />
              <small className="quiz-field-hint">از {questions.length} سوال</small>
            </div>
              ...
              <div className="quiz-setting-option">
                ...
                <div className="quiz-radio-group">
                  <label className={!settings.showAllQuestions ? "active" : ""}>
                    <input type="radio" name="show-mode" checked={!settings.showAllQuestions}
                      onChange={() => setSettings({ ...settings, showAllQuestions: false })} />
                    <span>یکی‌یکی</span>
                  </label>
                  <label className={settings.showAllQuestions ? "active" : ""}>
                    <input type="radio" name="show-mode" checked={settings.showAllQuestions}
                      onChange={() => setSettings({ ...settings, showAllQuestions: true })} />
                    <span>همه با هم</span>
                  </label>
                </div>
              </div>
              ...
              <div className="quiz-setting-option">
                ...
                <label className="quiz-switch">
                  <input
                    ...
                    onChange={(e) => setSettings({ ...settings, allowPreviousQuestion: e.target.checked })}
                  />
                  <span />
                </label>
              </div>
      ...
      <div className="quiz-navigation">
        ...
        <button type="button" className="btn quiz-save-btn" onClick={handleSaveQuiz} disabled={saving}>
          {saving ? (
            <><span className="spinner-border spinner-border-sm" />در حال ذخیره...</>
          ) : (
            <>ذخیره آزمون<i className="isax isax-tick-circle ms-1" /></>
          )}
        </button>
      </div>
          ...
          <div className="quiz-modal-wrapper" role="dialog" aria-modal="true">
            <div className="quiz-modal">
              <div className="quiz-modal-header">
                <div className="quiz-modal-title">
                  <div className="quiz-modal-icon">
                    <i className={editingClientId ? "isax isax-edit-2" : "fas fa-plus"} />
                  </div>
                  <div>
                ...
                <button type="button" className="quiz-modal-close" onClick={closeModal}>
              ...
              <div className="quiz-modal-body">
                ...
                <div className="quiz-modal-field">
                  <label className="form-label">
                    برچسب مهارتی <span className="text-muted fs-12 ms-1">(اختیاری)</span>
                  </label>
                  <input
                    type="text"
                    className="form-control"
                  ...
                  />
                  <small className="text-muted">این برچسب برای تحلیل شکاف مهارتی دانشجو استفاده می‌شود.</small>
                </div>
                ...
                <div className="quiz-choices-header">
                  ...
                  <span className="quiz-choice-count">{formChoices.length} گزینه</span>
                </div>
              ...
              <div className="quiz-modal-footer">
                <button type="button" className="btn quiz-modal-cancel" onClick={closeModal}>
                  انصراف
                </button>
                <button type="button" className="btn quiz-modal-save" onClick={saveQuestionFromModal}>
                  {editingClientId ? "ذخیره تغییرات" : "افزودن سوال"}
                </button>
              </div>
            </div>
          </div>
